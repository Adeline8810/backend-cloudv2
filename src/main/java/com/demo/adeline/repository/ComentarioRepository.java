...
public interface ComentarioRepository extends JpaRepository<Comentario, Long> {
	...
	@Query("SELECT new com.demo.adeline.model.ComentarioDTO(" +
	           ...
	           "c.fecha, " +
	           "c.usuarioId, " +
	           "u.username, " + // Antes: u.nombre
	           "u.fotoUrl, " +  // Antes: u.fotoPerfil / u.foto
	           "(SELECT COUNT(l.id) FROM ComentarioLike l WHERE l.comentarioId = c.id), " +
	           "(SELECT CASE WHEN COUNT(l.id) > 0 THEN true ELSE false END FROM ComentarioLike l WHERE l.comentarioId = c.id AND l.usuarioId = :usuarioLogueadoId)) " +
	           "FROM Comentario c " +
	           "JOIN Usuario u ON c.usuarioId = u.id " +
	           "WHERE c.videoId = :videoId " +
	           "ORDER BY c.fecha DESC")
	    List<ComentarioDTO> findComentariosConLikesYEstado(
